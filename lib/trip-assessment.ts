...
function normaliseAssessment(row: {
  ...
  return {
    ...
    threatScore: score,
    threatLevel: row.overall_level,
    recommendation: score >= 75 ? 'Avoid' : score >= 45 ? 'Go With Caution' : 'Go',
    confidence: row.confidence,
    confidenceScore: row.confidence === 'High' ? 78 : row.confidence === 'Medium' ? 58 : 35,
...
export async function buildTripAssessmentContext(trip: Trip) {
  ...
  return { country, city, documents, advisories, events, freshness, sources: sourceList(advisories, events, freshness) };
}
...
export async function assessTripRisk(trip: Trip): Promise<TripAssessmentRecord> {
  ...
  const aiAssessment = await generateItineraryRiskAssessment({ trip, assessment: risk, sources: context.sources });
  const createdAt = new Date().toISOString();
  const record: TripAssessmentRecord = { ...risk, id: randomUUID(), tripId: trip.id, aiAssessment, createdAt };
