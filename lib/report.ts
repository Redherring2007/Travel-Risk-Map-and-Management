...
export function generateTripReport(trip: Trip, documents: TripDocument[]): TripReport {
  ...
  const markdown = `# ${trip.name} Travel Risk Report\n\n` +
    ...
    `## 2. Overall Risk Rating\n${overall.meaning}\nConfidence: ${overall.confidence}. Sources: ${overall.sources.join(', ')}.\n\n` +
    `## 3. Destination Overview\nCapital: ${country.capital}. Region: ${country.region}. Population: ${country.population}. Languages: ${country.languages.join(', ')}. Currency: ${country.currency}.\n\n` +
    ...
    `## 19. Documents Checklist\nUploaded: ${documents.map((doc) => doc.type).join(', ') || 'None'}\nMissing: ${missing.join(', ') || 'None'}\n\n` +
  ...
  return { id: randomUUID(), tripId: trip.id, title: `${trip.name} Risk Report`, markdown, createdAt: new Date().toISOString(), recommendation };
}
