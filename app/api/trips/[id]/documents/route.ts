...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ data: store.listDocuments(id), storageMode: 'metadata-only; production file bytes live in S3-compatible storage and Neon stores audit metadata.' });
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doc = store.addDocument({ ...parsed.data, tripId: id, userId: user.id, storageProvider: 'demo', storageBucket: process.env.S3_BUCKET, storageKey });
